...
public class ConsentInterceptor {
	...
	public void interceptPreAccess(
		...
		IdentityHashMap<IBaseResource, ConsentOperationStatusEnum> authorizedResources =
			...
			for (int consentSvcIdx = 0; consentSvcIdx < myConsentService.size(); consentSvcIdx++) {
				...
				switch (outcome.getStatus()) {
					case PROCEED:
						break;
					case AUTHORIZED:
						authorizedResources.put(nextResource, ConsentOperationStatusEnum.AUTHORIZED);
						skipSubsequentServices = true;
						break;
					case REJECT:
						authorizedResources.put(nextResource, ConsentOperationStatusEnum.REJECT);
	...
	public void interceptPreShow(RequestDetails theRequestDetails, IPreResourceShowDetails thePreResourceShowDetails) {
		...
		for (int i = 0; i < thePreResourceShowDetails.size(); i++) {
			...
			if (resource == null
					|| authorizedResources.putIfAbsent(resource, ConsentOperationStatusEnum.PROCEED) != null) {
				continue;
			}
			...
			for (IConsentService nextService : myConsentService) {
				...
				switch (nextOutcome.getStatus()) {
					case PROCEED:
						...
						continue;
					case AUTHORIZED:
						authorizedResources.put(resource, ConsentOperationStatusEnum.AUTHORIZED);
						if (newResource != null) {
							thePreResourceShowDetails.setResource(i, newResource);
						}
						continue;
					case REJECT:
						authorizedResources.put(resource, ConsentOperationStatusEnum.REJECT);
						if (nextOutcome.getOperationOutcome() != null) {
							...
							authorizedResources.put(newOperationOutcome, ConsentOperationStatusEnum.PROCEED);
						} else {
							resource = null;
	...
	public void interceptOutgoingResponse(RequestDetails theRequestDetails, ResponseDetails theResponseDetails) {
		...
		if (alreadySeenResources.containsKey(theResponseDetails.getResponseResource())) {
			...
			ConsentOperationStatusEnum verdictOnResource =
					alreadySeenResources.get(theResponseDetails.getResponseResource());

			if (ConsentOperationStatusEnum.AUTHORIZED.equals(verdictOnResource)
					|| ConsentOperationStatusEnum.REJECT.equals(verdictOnResource)) {
				// the consent service verdict on the resource was AUTHORIZED or REJECT
				// in both cases, we can immediately return without checking children
				return;
			}
		} else {
			// we haven't seen this resource before
			// mark it as seen now, set the initial consent value to PROCEED by default, we will update if it changes
			// another value below
	 ...
	 * If the map contains a particular resource, it means that the resource has already been processed.
	 * If the value in the map is TRUE for a resource, it means that a consent service has returned AUTHORIZED
	 * for this resource.
